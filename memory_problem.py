...
def main(n_iter, n_batch, n_hidden, time_steps, learning_rate, savefile, model, input_type, out_every_t, loss_function, w_impl='urnn',n_reflections=None,flag_telescope=True,flag_useGivensForLoop=False,capacity=1,approx=False):
    ...
    if (model == 'LSTM'):           
        ...
        inputs, parameters, costs = EURNN(n_input, n_hidden, n_output,
                                            out_every_t=out_every_t,capacity=capacity,approx=approx,loss_function=loss_function)
        gradients = T.grad(costs[0], parameters)
...
if __name__=="__main__":
    ...
    parser.add_argument("--approx", default="False")
